...
- **Event Details (`#details`)**: Interactive 3D flip cards (total 5 events: Haldi, Engagement & Sangeet, Godh Bharai, Baraat & Jaimaal, Phere; 5th card centered) with dining notes ("Dinner to follow" / "Lunch to follow") and attire themes on back faces, 20s animated perimeter timer strokes, signature gold divider lines with center jewel dot, dual action buttons ("Save Event Details" & "How to reach the venue?") with 10% bottom viewport spacing, interactive venue navigation popup with QR code, and a dedicated itinerary modal featuring 2x2 grid layout, WhatsApp sharing, and custom themed PDF generation/downloading (`The_Wedding_Events_Mahek_Yash.pdf`).
